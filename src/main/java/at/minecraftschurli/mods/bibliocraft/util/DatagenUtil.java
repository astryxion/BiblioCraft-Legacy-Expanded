...
public final class DatagenUtil {
    ...
    public static <T> void addAll(Collection<? extends T> collection, TagAppender<T> tag) {
        for (T element : collection) {
            if (element instanceof Block block) {
                ((TagAppender<Block>) tag).add(block.builtInRegistryHolder().key());
            } else if (element instanceof Item item) {
                ((TagAppender<Item>) tag).add(item.builtInRegistryHolder().key());
            } else {
                throw new IllegalArgumentException("Unsupported tag element type: " + element.getClass());
            }
        }
    }
    ...
    /// @param <T>        The type of the collection elements.
    @SuppressWarnings("unchecked")
    public static <T> void addAllOptional(Collection<? extends T> collection, TagAppender<T> tag) {
        for (T element : collection) {
            if (element instanceof Block block) {
                ((TagAppender<Block>) tag).addOptional(block.builtInRegistryHolder().key());
            } else if (element instanceof Item item) {
                ((TagAppender<Item>) tag).addOptional(item.builtInRegistryHolder().key());
            } else {
                throw new IllegalArgumentException("Unsupported tag element type: " + element.getClass());
            }
        }
