...
public final class BlockModelDatagenUtil {
    ...
    public static final GroupedModelTemplate<BibliocraftWoodType> BOOKCASE = horizontalWoodTemplate(BCBlocks.BOOKCASE, BCModelTemplates.BOOKCASE);
    ...
    public static class ModelBuilder {
        ...
        public ModelBuilder withCustomBlockStateModelWrapper(Function<MultiVariant, CustomBlockStateModelBuilder> wrapper) {
            return withWrappedVariantFunction((UnaryOperator<MultiVariant>) variant -> wrapCustomBlockStateModel(wrapper.apply(variant)));
        }

        @SuppressWarnings("unchecked")
        private static MultiVariant wrapCustomBlockStateModel(CustomBlockStateModelBuilder builder) {
            try {
                var method = MultiVariant.class.getDeclaredMethod("of", CustomBlockStateModelBuilder.class);
                return (MultiVariant) method.invoke(null, builder);
            } catch (ReflectiveOperationException e) {
                throw new IllegalStateException("Custom block state models require NeoForge MultiVariant.of at runtime", e);
            }
        }
        ...
        private Identifier buildBlockModel(ModelTemplate template, TextureMapping textureMapping) {
            ...
            if (templateWrapper != null) {
                template = templateWrapper.apply(ExtendedModelTemplateBuilder.of(template)).build();
            }
        ...
        private static List<Weighted<CustomBlockStateModelBuilder>> getCustomBlockStateModels(MultiVariant multiVariant) {
            try {
                var method = MultiVariant.class.getDeclaredMethod("customBlockStateModels");
                @SuppressWarnings("unchecked")
                var weightedList = (net.minecraft.util.random.WeightedList<CustomBlockStateModelBuilder>) method.invoke(multiVariant);
        ...
        private ModelBuilder extractAndSetItemModelVariant(MultiVariant multiVariant) {
            ...
            if (list.isEmpty()) {
                List<Weighted<CustomBlockStateModelBuilder>> list1 = getCustomBlockStateModels(multiVariant);
