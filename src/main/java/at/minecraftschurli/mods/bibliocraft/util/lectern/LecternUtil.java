    ...
    public static boolean handleLecternUse(Level level, BlockPos pos, BlockState state, LecternBlockEntity lectern, Player player, InteractionHand hand) {
        ...
        if (book.isEmpty()) {
            ...
            if (stack.has(BCDataComponents.BIG_BOOK_CONTENT.get())) {
                lectern.pageCount = stack.get(BCDataComponents.BIG_BOOK_CONTENT.get()).pages().size();
            } else if (stack.has(BCDataComponents.WRITTEN_BIG_BOOK_CONTENT.get())) {
                lectern.pageCount = stack.get(BCDataComponents.WRITTEN_BIG_BOOK_CONTENT.get()).pages().size();
            ...
            if (player.isSecondaryUseActive()) {
                ...
                ServerPlayNetworking.send(sp, new OpenBookInLecternPacket(pos, book));
            }
            return true;
        }
        return false;
