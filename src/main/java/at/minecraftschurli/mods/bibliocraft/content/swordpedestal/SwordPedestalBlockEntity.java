...
public class SwordPedestalBlockEntity extends BCBlockEntity {
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, SwordPedestalBlockEntity blockEntity) {
        ...
        ItemStack stack = blockEntity.getItemHandler().getStack(0);
        if (!stack.isDamaged()) return;
        List<Holder<Enchantment>> list = stack.getEnchantments().keySet().stream()
        ...
        blockEntity.getItemHandler().set(0, ItemVariant.of(stack), stack.count());
    }
    ...
    public boolean isValid(int slot, ItemVariant stack) {
        return stack.isBlank() || stack.toStack(1).is(BCTags.Items.SWORD_PEDESTAL_SWORDS);
    }
    ...
    private int repairItem(ServerLevel level, ItemStack stack, int value) {
        int i = EnchantmentHelper.modifyDurabilityToRepairFromXp(level, stack, value);
