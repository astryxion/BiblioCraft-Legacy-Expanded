...
public class PrintingTableBindingTypewriterPagesRecipe extends PrintingTableBindingRecipe {
    ...
    public boolean matches(PrintingTableRecipeInput input, Level level) {
        ...
        for (ItemStack stack : input.left()) {
            if (!stack.isEmpty() && !stack.has(BCDataComponents.TYPEWRITER_PAGE.get())) return false;
        }
        return true;
    }
    ...
    public ItemStack assemble(PrintingTableRecipeInput input) {
        ...
        List<Filterable<Component>> list = input
                .left()
                .stream()
                .filter(e -> e.has(BCDataComponents.TYPEWRITER_PAGE.get()))
                .map(e -> concatTypewriterPageText(BCUtil.nonNull(e.get(BCDataComponents.TYPEWRITER_PAGE.get()))))
                .toList();
