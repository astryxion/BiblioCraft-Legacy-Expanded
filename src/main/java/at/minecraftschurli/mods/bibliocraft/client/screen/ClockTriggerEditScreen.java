...
public class ClockTriggerEditScreen extends Screen {
    ...
    protected void init() {
        ...
        final String[] lastValidHours = {""};
        final boolean[] updatingHours = {false};
        hours.setResponder(s -> {
            if (updatingHours[0]) {
                return;
            }
            boolean valid;
            ...
            if (valid) {
                ...
                // setValue always re-enters the responder in 26.2; restore last valid without looping
                updatingHours[0] = true;
                hours.setValue(lastValidHours[0]);
                updatingHours[0] = false;
            }
        });
        ...
        final String[] lastValidMinutes = {""};
        final boolean[] updatingMinutes = {false};
        minutes.setResponder(s -> {
            if (updatingMinutes[0]) {
                return;
            }
            boolean valid;
            try {
            ...
            if (valid) {
                lastValidMinutes[0] = s;
            } else {
                updatingMinutes[0] = true;
                minutes.setValue(lastValidMinutes[0]);
                updatingMinutes[0] = false;
            }
        });
