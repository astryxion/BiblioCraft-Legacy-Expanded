...
public class StockroomCatalogScreen extends Screen {
    ...
    private StockroomCatalogScreen(ItemStack stack, Player player, @Nullable InteractionHand hand, @Nullable BlockPos lectern) {
        ...
        this.data = stack.getOrDefault(BCDataComponents.STOCKROOM_CATALOG_CONTENT.get(), StockroomCatalogContent.DEFAULT);
        requestPacket();
    }
    ...
    private void requestPacket() {
        if (hand != null) {
            ClientPlayNetworking.send(new StockroomCatalogRequestListPacket(containerSorting, itemSorting, Either.left(hand)));
        } else if (lectern != null) {
            ClientPlayNetworking.send(new StockroomCatalogRequestListPacket(containerSorting, itemSorting, Either.right(lectern)));
        }
    }
    ...
    private void setDataOnStack() {
        ...
        if (hand != null) {
            ClientPlayNetworking.send(new StockroomCatalogSyncPacket(data, Either.left(hand)));
        } else if (lectern != null) {
            ClientPlayNetworking.send(new StockroomCatalogSyncPacket(data, Either.right(lectern)));
        }
    }
    ...
    private static class SortButton<E extends Enum<E> & StockroomCatalogSorting> extends Button.Plain {
        ...
        public SortButton(E initialValue, int x, int y, int width, int height, OnPress onPress) {
            super(x, y, width, height, message(initialValue), onPress, DEFAULT_NARRATION);
            value = initialValue;
        }
