...
public record TableBlockStateModel(BlockStateModel base, Map<DyeColor, Map<TableBlock.Type, BlockStateModelPart>> cloths) implements DynamicBlockStateModel {
    ...
    public void collectParts(BlockAndTintGetter level, BlockPos pos, BlockState state, RandomSource random, List<BlockStateModelPart> parts) {
        ...
        if (base instanceof DynamicBlockStateModel dynamic) {
            dynamic.collectParts(level, pos, state, random, parts);
        } else {
            base.collectParts(random, parts);
        }
        DyeColor color = level.getBlockEntity(pos) instanceof TableBlockEntity table ? table.getClothColor() : null;
    ...
    public @BakedQuad.MaterialFlags int materialFlags(BlockAndTintGetter level, BlockPos pos, BlockState state) {
        if (base instanceof net.neoforged.neoforge.client.extensions.BlockStateModelExtension extension) {
            return extension.materialFlags(level, pos, state);
        }
        return base.materialFlags();
    }
    ...
    public Object createGeometryKey(BlockAndTintGetter level, BlockPos pos, BlockState state, RandomSource random) {
        return new GeometryKey(state, level.getBlockEntity(pos) instanceof TableBlockEntity table ? table.getClothColor() : null);
    }
    ...
    public record Unbaked(SingleVariant.Unbaked base, Map<DyeColor, Map<TableBlock.Type, Identifier>> cloths) implements CustomUnbakedBlockStateModel {
        public static final MapCodec<Unbaked> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group(
                SingleVariant.Unbaked.CODEC.fieldOf("base").forGetter(Unbaked::base)
