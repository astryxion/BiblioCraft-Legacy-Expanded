...
public class BigBookScreen extends Screen {
    ...
    private BigBookScreen(ItemStack stack, Player player, @Nullable InteractionHand hand, @Nullable BlockPos lectern) {
        ...
        if (stack.has(BCDataComponents.WRITTEN_BIG_BOOK_CONTENT.get())) {
            WrittenBigBookContent content = BCUtil.nonNull(stack.get(BCDataComponents.WRITTEN_BIG_BOOK_CONTENT.get()));
            pages = new ArrayList<>(content.pages());
            currentPage = content.currentPage();
            writable = false;
        } else if (stack.has(BCDataComponents.BIG_BOOK_CONTENT.get())) {
            BigBookContent content = BCUtil.nonNull(stack.get(BCDataComponents.BIG_BOOK_CONTENT.get()));
            pages = new ArrayList<>(content.pages());
            currentPage = content.currentPage();
    ...
    protected void init() {
        ...
        if (isSigning) {
            ...
            colorBox.setResponder(s -> {
                if (!s.isEmpty() && (s.charAt(0) != '#' || !s.substring(1).matches("[0-9a-fA-F]*"))) {
            ...
            for (int i = 0; i < colors.length; i++) {
                ...
                ColorButton button = addRenderableWidget(new ColorButton(TextColor.fromLegacyFormat(colors[i]).getValue(), rightX + 80 - 16 * (4 - i % 4), 80 + 16 * Math.floorDiv(i, 4), 16, 16, Component.translatable("color." + colors[i].name().toLowerCase()), $ -> setColor(TextColor.fromLegacyFormat(colors[j]).getValue())));
                button.setTooltip(Tooltip.create(Component.translatable("color." + colors[i].name().toLowerCase())));
            }
            ...
            sizeBox.setResponder(s -> {
                ...
                try {
                    ...
                    valid = i >= FormattedLine.MIN_SIZE && i <= FormattedLine.MAX_SIZE;
            ...
            backButton = addRenderableWidget(new PageButton(leftX + 83, BACKGROUND_HEIGHT - 32, false, $ -> {
                ...
                updateButtonVisibility();
                if (lectern != null) {
                    ClientPlayNetworking.send(new SetBigBookPageInLecternPacket(currentPage, Either.right(lectern)));
                }
            }, true));
            ...
            if (lectern != null) {
                addRenderableWidget(Button.builder(Translations.VANILLA_TAKE_BOOK, button -> {
                            ...
                            ClientPlayNetworking.send(new TakeLecternBookPacket(lectern));
                        })
    ...
    public void onClose() {
        ...
        if (writable) {
            ...
            stack.set(BCDataComponents.BIG_BOOK_CONTENT.get(), content);
            ClientPlayNetworking.send(new BigBookSyncPacket(content, BCUtil.nonNull(hand)));
        } else if (hand != null) {
            ClientPlayNetworking.send(new SetBigBookPageInLecternPacket(currentPage, Either.left(hand)));
    ...
    private void finalizeBook() {
        ...
        ClientPlayNetworking.send(new BigBookSignPacket(content, hand));
        super.onClose();
    }
