...
public class ClipboardScreen extends Screen {
    ...
    public ClipboardScreen(ItemStack stack, InteractionHand hand) {
        ...
        this.stack = stack;
        this.hand = hand;
        this.data = stack.getOrDefault(BCDataComponents.CLIPBOARD_CONTENT.get(), ClipboardContent.DEFAULT);
    }

    @Override
    public void onClose() {
        super.onClose();
        stack.set(BCDataComponents.CLIPBOARD_CONTENT.get(), data);
        ClientPlayNetworking.send(new ClipboardSyncPacket(data, hand));
    }

    @Override
    protected void init() {
        int x = (width - 192) / 2;
        titleBox = addRenderableWidget(new EditBox(minecraft.font, x + 57, 14, 72, 8, Component.empty()));
        ...
        for (int i = 0; i < ClipboardContent.MAX_LINES; i++) {
            ...
            lines[i] = addRenderableWidget(new EditBox(minecraft.font, x + 45, 15 * i + 28, 109, 8, Component.empty()));
