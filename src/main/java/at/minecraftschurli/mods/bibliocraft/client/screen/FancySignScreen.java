...
public class FancySignScreen extends Screen {
    ...
    public void onClose() {
        ...
        ClientPlayNetworking.send(new FancySignSyncPacket(list, pos, back));
        super.onClose();
    }
    ...
    protected void init() {
        ...
        for (int i = 0; i < colors.length; i++) {
            ...
            ColorButton button = addRenderableWidget(new ColorButton(TextColor.fromLegacyFormat(colors[i]).getValue(), rightX + 80 - 16 * (4 - i % 4), y + 16 * Math.floorDiv(i, 4), 16, 16, Component.translatable("color." + colors[i].name().toLowerCase()), $ -> setColor(TextColor.fromLegacyFormat(colors[j]).getValue())));
