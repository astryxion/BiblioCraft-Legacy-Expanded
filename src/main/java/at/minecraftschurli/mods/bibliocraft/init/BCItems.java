...
import at.minecraftschurli.mods.bibliocraft.util.registrar.DeferredBlock;
import at.minecraftschurli.mods.bibliocraft.util.registrar.DeferredItem;
...
public interface BCItems {
    ...
    DeferredItem<BigBookItem>          BIG_BOOK               = registerItem("big_book", BigBookItem::new, p -> p.stacksTo(1).component(DataComponents.MAX_STACK_SIZE, 1).component(BCDataComponents.BIG_BOOK_CONTENT.get(), BigBookContent.DEFAULT));
    DeferredItem<BigBookItem>          WRITTEN_BIG_BOOK       = registerItem("written_big_book", BigBookItem::new, p -> p.stacksTo(1).component(DataComponents.MAX_STACK_SIZE, 1).component(DataComponents.ENCHANTMENT_GLINT_OVERRIDE, true).component(BCDataComponents.WRITTEN_BIG_BOOK_CONTENT.get(), WrittenBigBookContent.DEFAULT));
    ...
    DeferredItem<TypewriterPageItem>   TYPEWRITER_PAGE        = registerItem("typewriter_page", TypewriterPageItem::new, p -> p.stacksTo(1).component(BCDataComponents.TYPEWRITER_PAGE.get(), TypewriterPage.DEFAULT));
    //TODO Hand Drill
    //TODO Screw Gun
    //TODO Monocle
    ...
    private static DeferredItem<BlockItem> registerBlockItem(DeferredBlock<? extends Block> block, UnaryOperator<Item.Properties> properties) {
    ...
    private static <I extends BlockItem> DeferredItem<I> registerBlockItem(DeferredBlock<? extends Block> block, BiFunction<Block, Item.Properties, I> factory, UnaryOperator<Item.Properties> properties) {
        DeferredItem<I> item = BCRegistries.ITEMS.registerItem(block.getId().getPath(), p -> factory.apply(block.get(), p), properties);
        OTHER.add(item);
        return item;
    }
