...
public class FancyLanternBlock extends AbstractFancyLightBlock {
    ...
    private ParticleOptions getParticle() {
        var p = BuiltInRegistries.PARTICLE_TYPE.get(particleId);
        return p instanceof ParticleOptions o ? o : ParticleTypes.FLAME;
    }
    ...
    protected ItemInteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hitResult) {
        ...
        if (!state.getValue(LIT) && (stack.getItem() instanceof FlintAndSteelItem || stack.is(Items.FIRE_CHARGE))) {
            if (player.getAbilities().mayBuild) {
                ...
                level.gameEvent(player, GameEvent.BLOCK_CHANGE, pos);
                if (!player.getAbilities().instabuild && stack.getItem() instanceof FlintAndSteelItem) {
                ...
                if (!player.getAbilities().instabuild && stack.is(Items.FIRE_CHARGE)) {
                    stack.shrink(1);
                }
                return ItemInteractionResult.sidedSuccess(level.isClientSide);
            }
