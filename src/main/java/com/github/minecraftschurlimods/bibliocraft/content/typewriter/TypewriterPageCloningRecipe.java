...
public class TypewriterPageCloningRecipe extends CustomRecipe {
    ...
    public ItemStack assemble(CraftingInput input, HolderLookup.Provider registries) {
        ...
        if (!stack.has(BCDataComponents.TYPEWRITER_PAGE.get())) return ItemStack.EMPTY;
        ItemStack result = stack.copyWithCount(paper);
        result.set(BCDataComponents.TYPEWRITER_PAGE.get(), stack.get(BCDataComponents.TYPEWRITER_PAGE.get()));
        return result;
    }
    ...
    public NonNullList<ItemStack> getRemainingItems(CraftingInput input) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (stack.getItem().getCraftingRemainingItem() != net.minecraft.world.item.Items.AIR) {
                list.set(i, new net.minecraft.world.item.ItemStack(stack.getItem().getCraftingRemainingItem(), 1));
