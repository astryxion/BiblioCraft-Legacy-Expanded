...
public class TableBER implements BlockEntityRenderer<TableBlockEntity> {
    public TableBER(BlockEntityRendererProvider.Context ctx) {}
    ...
    /**
     * Rebuilds the table cloth model cache. Safe to call from render thread after models have loaded.
     * No-op if Minecraft or ModelManager is not ready (e.g. during model baking).
     */
    public static void rebuildClothModelCache() {
        Minecraft mc = ClientUtil.getMc();
        if (mc == null) return;
        ...
        for (TableBlock.Type type : TableBlock.Type.values()) {
            ...
            for (DyeColor color : DyeColor.values()) {
                BakedModel baked = models.getModel(new ModelResourceLocation(BCUtil.bcLoc("block/color/" + color.getSerializedName() + "/table_cloth_" + type.getSerializedName()), "inventory"));
                if (baked != null) map.put(color, baked);
            }
    ...
    public static void clearClothModelCache() {
        CLOTH_MAP.clear();
    }
    ...
    public void render(TableBlockEntity blockEntity, float partialTick, PoseStack stack, MultiBufferSource buffer, int light, int overlay) {
        ...
        if (carpet.isEmpty() || !(carpet.getItem() instanceof BlockItem bi && bi.getBlock() instanceof WoolCarpetBlock carpetBlock))
            return;
        if (CLOTH_MAP.isEmpty()) rebuildClothModelCache();
        BlockState state = blockEntity.getBlockState();
        Map<DyeColor, BakedModel> typeMap = CLOTH_MAP.get(state.getValue(TableBlock.TYPE));
        BakedModel clothModel = typeMap != null ? typeMap.get(carpetBlock.getColor()) : null;
        if (clothModel == null) return;
        stack.pushPose();
        ...
        stack.translate(-0.5, 0, -0.5);
        ClientUtil.renderBakedModel(clothModel, stack, buffer, blockEntity.getLevel(), blockEntity.getBlockPos(), state, RANDOM, BlockModelData.EMPTY);
        stack.popPose();
    }
}
