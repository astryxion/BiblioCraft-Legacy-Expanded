...
public class BookcaseModel extends DynamicBlockModel {
    ...
    public static class BookcaseGeometry implements IUnbakedGeometry<BookcaseGeometry> {
        ...
        public BakedModel bake(IGeometryBakingContext context, ModelBaker baker, Function<Material, TextureAtlasSprite> spriteGetter, ModelState modelState, ItemOverrides overrides, ResourceLocation modelId) {
            BakedModel base = new ElementsModel(this.base.getElements()).bake(context, baker, spriteGetter, modelState, overrides, modelId);
            ...
            for (int j = 0; j < books.length; j++) {
                ResourceLocation bookId = modelId.withSuffix("_book_" + j);
                books[j] = this.books[j].bake(baker, spriteGetter, bookState, bookId);
            }
