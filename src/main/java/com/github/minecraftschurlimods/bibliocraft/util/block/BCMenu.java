...
public abstract class BCMenu<T extends BCMenuBlockEntity> extends AbstractContainerMenu {
     ...
     * @param id          The id of the menu, provided by the game.
     * @param inventory   The {@link Inventory} to use, provided by the game.
     * @param pos         The {@link BlockPos} of the block entity (e.g. from Fabric extended screen handler data).
     */
    @SuppressWarnings("unchecked")
    public BCMenu(MenuType<?> type, int id, Inventory inventory, BlockPos pos) {
        this(type, id, inventory, (T) BCUtil.nonNull(getLevel(inventory).getBlockEntity(pos)));
    }

    private static Level getLevel(Inventory inventory) {
